  ...
  <div className="w-full h-full min-h-[300px] sm:min-h-[360px] md:min-h-[400px]">
    <Suspense fallback={<LoadingSpinner size="md" text="Loading 3D cube..." />}>
        <Canvas
          ...
          gl={{ powerPreference: 'high-performance', antialias: false, alpha: false, preserveDrawingBuffer: false }}
          dpr={[1, 1.5]}
          ...
          onCreated={({ gl }) => {
            ...
            canvas.addEventListener('webglcontextlost', prevent, false);
            canvas.addEventListener('webglcontextrestored', prevent, false);
          }}
        >
          <color attach="background" args={["#ffffff"]} />
          <ambientLight intensity={0.8} />
          <directionalLight position={[10, 15, 20]} intensity={1} />
          ...
          <OrbitControls enablePan={false} enableZoom={false} enableRotate={true} autoRotate={autoRotate} autoRotateSpeed={1.0} />
      </Canvas>
    </Suspense>
  </div>
);
